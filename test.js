const bibleStructure = {
  ...
  열왕기하: [
    18, 25, 27, 44, 27, 33, 20, 29, 37, 36, 20, 21, 25, 29, 38, 20, 41, 37, 37,
    21, 26, 20, 37, 20, 30,
  ],
  역대상: [
  ...
  느헤미야: [11, 20, 38, 17, 19, 19, 73, 18, 38, 39, 36, 47, 31],
...
const handleInputChange = (verse, content) => {
  // 절 데이터를 출력하거나 저장
  console.log(`절 입력: ${content}`);
};

// Node.js에서 성경 읽기 시뮬레이션 실행
const simulateFullReading = async () => {
  console.log("성경 읽기 테스트 시작");

  for (const [book, chapters] of Object.entries(bibleStructure)) {
    console.log(`${book} 입력 시작`);

    for (let chapterIndex = 0; chapterIndex < chapters.length; chapterIndex++) {
      const versesInChapter = chapters[chapterIndex];

      console.log(`${book} ${chapterIndex + 1}장 (${versesInChapter}절) 시작`);

      for (let verse = 1; verse <= versesInChapter; verse++) {
        const content = `${book} ${chapterIndex + 1}:${verse} 테스트 입력`;
        handleInputChange(verse, content); // 절 입력 처리
      }

      console.log(`${book} ${chapterIndex + 1}장 완료`);
    }

    console.log(`${book} 완료!`);
  }

  console.log("🎉 성경 읽기 테스트 완료!");
};
